...
Tests live in `tests/vitest/`. The runner is **Vitest**. Tests run with `yarn test` (or `rtk vitest run`).

## tests/vitest/init.ts

This file runs before every test suite. Its job is to populate `process.env` with valid dummy values for all **required** config keys (those without `.optional()` in the Zod schema).
