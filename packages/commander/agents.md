...
- `make:page --register` can mutate multiple files transitively if `--selectors`, `--event`, `--handler`, or transition handler flags are also enabled, because registration options propagate to generated add-ons
