...
- `--listeners-path` → folder for listener `.ts` when `--listeners` is set. Default: `./src/app/Listeners/`
...
- `--listeners` requires `--listeners-path` (or rely on the CLI default); if the path is omitted in programmatic calls, no listener file is created.
